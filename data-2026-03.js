const posts_202603 = [
  {
    id: 3,
    date: "2026-03-22T15:30",
    weiboUrl: "https://weibo.com/5994946077/5279276314135607",
    originalText: "hello ​​​",
    translatedText: "hello",
    images: [],
    comments: [
      {
        id: 302,
        username: "哭泣鱼丸丸丸-",
        originalText: "Hi！好看吗！小然",
        translatedText: "Hi！익연아 예쁘지?",
        images: ["images/sunset-3shot.jpg"],
        actorReply: { originalText: "手机拍的吗", translatedText: "폰으로 찍은 거야?" }
      },
      {
        id: 303,
        username: "烟竹阮桐粥",
        originalText: "轮家庭地位这一块[思考]",
        translatedText: "집안 서열에 대해서는 말이지[思考]",
        images: ["images/family-3shot.jpg"],
        actorReply: { originalText: "？？", translatedText: "？？" }
      },
      {
        id: 304,
        username: "yi翊口小周",
        originalText: "哥哥 法拉利也来开赛车啦！！",
        translatedText: "오빠 페라리도 레이싱 하러 왔대!!",
        images: ["images/ferrari-racing.jpg"],
        actorReply: { originalText: "牛………..", translatedText: "대박이네………." }
      },
      {
        id: 305,
        username: "樱花味的cherry",
        originalText: "回重庆吃啥好吃的没有啊？[期待]",
        translatedText: "충칭 왔을 때 맛있는 거 뭐 먹었어?[期待]",
        images: ["images/chongqing-bridge.jpg", "images/chongqing-food.jpg"],
        actorReply: { originalText: "你这家店怎么很熟悉", translatedText: "여기 왜 이렇게 낯이 익지" }
      },
      {
        id: 306,
        username: "r男朋友真的只想开车",
        originalText: "是不是偷偷去学画画了[二哈]",
        translatedText: "혹시 몰래 그림 배우러 다녔나[二哈]",
        images: ["images/sunflower-drawing.jpg", "images/sunflower-drawing2.jpg"],
        actorReply: { originalText: "是不是画得不错[doge]", translatedText: "내가 그림 좀 잘 그리긴해[doge]" }
      },
      {
        id: 307,
        username: "轻轻茉莉不加冰-",
        originalText: "小然 好奇上次vlog里这个角度是咋拍的！🤔",
        translatedText: "익연아 궁금한 게 지난 브이로그에서 이 각도는 어떻게 찍은 거야!🤔",
        images: ["images/paris-vlog.jpg"],
        actorReply: { originalText: "把相机放在地上 拍完再跑回去捡[航天员]", translatedText: "카메라를 바닥에 두고 찍은 다음에 다시 뛰어가서 주워왔어[航天员]"}
      },
      {
        id: 308,
        username: "脆皮灿包",
        originalText: "来看日落咯",
        translatedText: "일몰 보러 왔어",
        images: ["images/sea-wave.jpg"],
        actorReply: { originalText: "拍的真好 手机拍的吗", translatedText: "진짜 잘 찍었다 폰으로 찍은거야?" }
      },
      {
        id: 309,
        username: "然后煮翊碗周",
        originalText: "然然快看 昨天一块钱转盘转到一只小乌龟🐢 可爱不[期待]",
        translatedText: "익연 빨리 봐봐 어제 1위안짜리 뽑기 돌려서 꼬마 거북이 한 마리 뽑았어🐢 귀엽지[期待]",
        images: ["images/turtle.jpg"],
        actorReply: { originalText: "我小时候也抽到过这个", translatedText: "나도 어릴 때 이거 뽑아본 적 있는데" }
      },
      {
        id: 310,
        username: "nuori纯",
        originalText: "然然每次给yummy画的妆怎么都是一样的",
        translatedText: "익연이는 yummy한테 메이크업해 줄 때마다 왜 다 똑같은 거야",
        images: ["images/apm-bear.jpg"],
        actorReply: { originalText: "实在想不到其他的了", translatedText: "진짜 다른 게 안 떠올라서 그래" }
      },
      {
        id: 311,
        username: "翊粒紫然Tz",
        originalText: "谢谢周哥的翻牌，虽然还没翻，可能是忘了，但是提前谢谢了，周哥总不可能真的不翻吧，肯定不可能，毕竟哥哥超级心软，我怕一会翻我的时候哥哥看不到의 my thank you, 这里提前谢谢全宇宙超级无敌第一帅的哥哥‼️你最棒‼️",
        translatedText: "오빠 답글 고마워, 비록 아직 안 달아줬고 까먹었을 수도 있지만, 그래도 미리 고마워, 우리 오빠가 설마 답글을 안 달아줄 리가 없잖아, 절대 그럴 리 없지, 왜냐면 오빠는 엄청 마음이 약하니까, 이따가 나한테 답글 달아줄 때 오빠가 내 감사 인사를 못 볼까 봐 미리 전 우주에서 제일 최고로 잘생긴 오빠한테 감사 인사를 남겨‼️ 네가 최고야‼️",
        images: ["images/shaving.jpg"],
        actorReply: { originalText: "来了啊", translatedText: "그래 나왔어" }
      },
      {
        id: 312,
        username: "藤孥Love-翊",
        originalText: "好玩吗这个迷你版赛车🙃",
        translatedText: "재밌어? 미니 레이싱카🙃",
        images: ["images/racing-assembly.jpg"],
        actorReply: { originalText: "拼起来要点时间[doge]", translatedText: "조립하는 데 시간 좀 걸리더라[doge]" }
      },
      {
        id: 313,
        username: "翊口韩式拌饭-Tz",
        originalText: "哥哥！耳洞打的时候疼不疼啊 我也想打 但怕疼 推荐一个给我😚",
        translatedText: "오빠！귀 뚫을 때 아파? 나도 뚫고 싶은데 아플 것 같아서 무서웡 어디 뚫을까 추천해 줘😚",
        images: ["images/ear-gun.jpg"],
        actorReply: { originalText: "你打和我一个位置应该是最不疼的", translatedText: "너도 나랑 같은 위치에 뚫어 그럼 안 아플 거야" }
      },
      {
        id: 314,
        username: "我那比不直耶的男朋友",
        originalText: "然然 好奇手里是什么",
        translatedText: "익연아 궁금한 게 있는데 손에 들고 있는 거 뭐야?",
        images: ["images/hand-food.jpg"],
        actorReply: { originalText: "牛肉干！", translatedText: "육포！" }
      },
      {
        id: 315,
        username: "JuscHe",
        originalText: "好看的很[doge]",
        translatedText: "개멋있지[doge]",
        images: ["images/cart-3shot.jpg"],
        actorReply: { originalText: "出门就能看f1是吧[二哈]", translatedText: "집 밖에만 나가면 바로 F1 볼 수 있겠네[二哈]" }
      },
      {
        id: 316,
        username: "周了个粥呀",
        originalText: "第一次发现你这么有绘画天赋",
        translatedText: "너가 이렇게 그림에 재능이 있는지 처음 알았어",
        images: ["images/sunflower-1shot.jpg"],
        actorReply: { originalText: "真的假的？？", translatedText: "진심이야？？" }
      },
      {
        id: 317,
        username: "只修兔纸Tz",
        originalText: "哥哥去重庆吃好吃的了吗❤️",
        translatedText: "오빠 충칭 가서 맛있는 거 먹었어?❤️",
        images: ["images/chongqing-noodle.jpg"],
        actorReply: { replyImages: ["images/jeonbyeong-food.jpg"] }
      },
      {
        id: 318,
        username: "白月光的月_tz",
        originalText: "如何🏎️？",
        translatedText: "레이싱 어때🏎️？",
        images: ["images/labubu-racing.jpg"],
        actorReply: { originalText: "", translatedText: "", images: ["images/jeonbyeong-food.jpg"] }
      },
      {
        id: 319,
        username: "YZ翊小楠",
        originalText: "哥哥你是不是最近偷偷健身了",
        translatedText: "오빠 최근에 몰래 운동 했지",
        images: ["images/chongqing-v3.jpg"],
        actorReply: { originalText: "角度问题", translatedText: "각도빨이야" }
      },
      {
        id: 320,
        username: "落知梦Tz",
        originalText: "说，这个视频你拍了多久？",
        translatedText: "대답해봐, 너 이 영상 찍는 데 얼마나 걸렸어 오래 걸렸지?",
        images: ["images/charles-challenge.jpg"],
        actorReply: { originalText: "一遍！", translatedText: "1번！" }
      },
      {
        id: 321,
        username: "yisiyiiiiii",
        originalText: "看这是谁",
        translatedText: "얘 누구게",
        images: ["images/red-cart.jpg"],
        actorReply: { originalText: "这个人看起来应该挺会开赛车的", translatedText: "딱 봐도 레이싱 엄청 잘하게 생겼네" }
      },
      {
        id: 322,
        username: "追十七秒的日落",
        originalText: "哥哥 这次回重庆拍摄悦游好吗！快推荐一些重庆打卡地方[奥特曼]",
        translatedText: "오빠 이번에 충칭 가서 트래블러(잡지) 촬영한 거 좋았어! 충칭에 가볼 만한 곳 몇 군데만 추천해 주라[奥特曼]",
        images: [],
        actorReply: { originalText: "大扶梯！ 真的很厉害 我还是第一次去", translatedText: "皇冠大扶梯(대형 에스컬레이터)！ 진짜 대박이더라 나도 이번에 처음 가봤어" }
      },
      {
        id: 323,
        username: "喝口QQ粥-",
        originalText: "哥哥你尚博勒的评分多少呀[航天员] ",
        translatedText: "오빠 체임버 점수 몇 점이야[航天员] ",
        images: ["images/chamber-card.jpg"],
        actorReply: { originalText: "400往上[doge]", translatedText: "400점 이상[doge]" }
      },
      {
        id: 324,
        username: "夜雨愀",
        originalText: "哥哥和偶像拍手势舞是啥感觉[期待]",
        translatedText: "오빠 우상이랑 같이 제스처 챌린지 찍을 때 기분이 어땠어[期待]",
        images: [],
        actorReply: { originalText: "爽！！！！", translatedText: "쩔었지！！！！" }
      },
      {
        id: 301,
        username: "周翊然Tz",
        originalText: "走啦 下次见各位🫡",
        translatedText: "갈게 담에 봐 얘들아[航天员] ",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 4,
    date: "2026-03-03T23:07",
    weiboUrl: "https://weibo.com/5994946077/5272505997854820",
    originalText: "元宵节快乐",
    translatedText: "즐거운 원소절 보내",
    images: ["images/yuanxiao.jpg"],
    comments: []
  }
];
