const posts_202510 = [
  {
    id: 251001,
    date: "2025-10-22",
    weiboUrl: "https://weibo.com/5994946077/QakJun9Yo",
    authorName: "周翊然Tz",
    originalText: "hi 我来了",
    translatedText: "hi 나 왔다",
    images: [],
    comments: [
      {
        id: 25100102,
        username: "扁桃体发炎不发言-",
        originalText: "哥哥！请问唐亦寻他第一次开赛车紧张吗哈哈哈哈😆",
        translatedText: "오빠! 탕이쉰으로 처음 레이싱 하는건데 긴장했어? ㅋㅋㅋㅋ😆",
        images: ["images/2510-first-race.jpg"],
        actorReply: {
          originalText: "还行",
          translatedText: "할 만했어"
        }
      },
      {
        id: 25100103,
        username: "咬一口脆皮年糕_",
        originalText: "哥哥 给你分享我做的立体纸艺😄",
        translatedText: "오빠 내가 만든 입체 종이공예 보여줄게😄",
        images: ["images/2510-paper-craft.jpg"],
        actorReply: {
          originalText: "好厉害",
          translatedText: "진짜 잘한다"
        }
      },
      {
        id: 25100104,
        username: "魚肉蛋黄酥o_",
        originalText: "法拉利在赛车场享受按摩这事儿它跟你说了吗？🐶",
        translatedText: "페라리가 서킷에서 마사지 받은 거, 페라리가 너한테 말했어?🐶",
        images: ["images/2510-ferrari-massage.jpg"],
        actorReply: {
          originalText: "听说了",
          translatedText: "들었어"
        }
      },
      {
        id: 25100105,
        username: "来翊口甜瓜",
        originalText: "帮我选一个口味[你好]",
        translatedText: "맛 하나 골라줘[你好]",
        images: ["images/2510-pick-a-flavor.jpg"],
        actorReply: {
          originalText: "蓝莓",
          translatedText: "블루베리"
        }
      },
      {
        id: 25100106,
        username: "牙不吃香菜",
        originalText: "周哥有钓到什么吗",
        translatedText: "오빠 뭐 낚은 거 있어?",
        images: ["images/2510-fishing-catch.jpg"],
        actorReply: {
          originalText: "大鱼和小鱼",
          translatedText: "큰 물고기랑 작은 물고기"
        }
      },
      {
        id: 25100107,
        username: "魚肉蛋黄酥o_",
        originalText: "老公 听说你最近在钓鱼🎣有没有什么收获可以分享一下呢？🥰",
        translatedText: "자기야 요즘 낚시한다며🎣 뭐 건진 거 있으면 공유 해주면 안 돼?🥰",
        images: ["images/2510-fishing-share.jpg"],
        actorReply: {
          originalText: "还在钻研中",
          translatedText: "아직 연구 중이야"
        }
      },
      {
        id: 25100108,
        username: "XJJJXI",
        originalText: "我还在吃饭呢 你吃饭了吗",
        translatedText: "나 아직 밥 먹는 중인데 너는 밥 먹었어?",
        images: ["images/2510-still-eating.jpg"],
        actorReply: {
          originalText: "还没",
          translatedText: "아직"
        }
      },
      {
        id: 25100109,
        username: "吧唧一口然然的脸蛋_zyrzyr",
        originalText: "天冷[泪]记得多穿",
        translatedText: "날씨 너무 춥다[泪] 옷 든든히 입어",
        images: [],
        actorReply: {
          originalText: "大家也要多穿点",
          translatedText: "너네도 옷 든든히 입고다녀"
        }
      },
      {
        id: 25100110,
        username: "碎碎念nvn",
        originalText: "哥哥看日落！",
        translatedText: "오빠 노을 봐봐!",
        images: ["images/2510-sunset.jpg"],
        actorReply: {
          originalText: "wow",
          translatedText: "wow"
        }
      },
      {
        id: 25100111,
        username: "芒果粥z",
        originalText: "哥哥看雪",
        translatedText: "오빠 눈 봐봐",
        images: ["images/2510-snow.jpg"],
        actorReply: {
          originalText: "都开始下雪了？",
          translatedText: "벌써 눈이 와?"
        }
      },
      {
        id: 25100112,
        username: "小吴同学KC",
        originalText: "然哥快看大熊！超级可爱[哇]",
        translatedText: "오빠 빨리 큰 곰돌이 좀 봐봐! 완전 귀엽지[哇]",
        images: ["images/2510-big-bear.jpg"],
        actorReply: {
          originalText: "哈哈哈感觉还没睡醒",
          translatedText: "ㅋㅋㅋ 아직 잠에서 덜 깬 것 같은데"
        }
      },
      {
        id: 25100101,
        username: "周翊然Tz",
        originalText: "我走了 明天见",
        translatedText: "가볼게 내일 보자",
        images: [],
        actorReply: null
      }
    ]
  }
];
