const posts_202412 = [
  {
    id: 241201,
    date: "2024-12-01",
    weiboUrl: "https://weibo.com/5994946077/P2Rxuit2M",
    originalText: "好久不见",
    translatedText: "오랜만이야",
    images: [],
    comments: []
  },
  {
    id: 241202,
    date: "2024-12-01",
    weiboUrl: "https://weibo.com/6462240253/P2RyBjoFV",
    authorName: "-xixi陈-",
    authorAvatar: "🥣",
    originalText: "然然快看！",
    translatedText: "익연아 얼른 봐봐!",
    note: "📹 원문 동영상",
    images: [],
    comments: [
      {
        id: 24120201,
        username: "周翊然Tz",
        originalText: "快出残影了",
        translatedText: "잔상 남겠는데",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241203,
    date: "2024-12-01",
    weiboUrl: "https://weibo.com/7901364565/P2RyvdMfH",
    authorName: "只想见翊面-",
    authorAvatar: "🥣",
    originalText: "小狗，可爱吗",
    translatedText: "강아지, 귀엽지?",
    images: ["images/2412-dog.jpg"],
    comments: [
      {
        id: 24120301,
        username: "周翊然Tz",
        originalText: "法拉利雪人版？",
        translatedText: "페라리 눈사람 버전이야?",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241204,
    date: "2024-12-01",
    weiboUrl: "https://weibo.com/7926770592/P2RzYpsEn",
    authorName: "玉米小甜粥-",
    authorAvatar: "🥣",
    originalText: "小然 看我的电动车可爱嘛 你喜欢的蜡笔小新哦",
    translatedText: "익연아 내 전동스쿠터 귀엽지? 너가 좋아하는 짱구야",
    images: ["images/2412-ebike.jpg"],
    comments: []
  },
  {
    id: 241205,
    date: "2024-12-01",
    weiboUrl: "https://weibo.com/5034575256/P2RAjuQBy",
    authorName: "然然的大梦想家",
    authorAvatar: "🥣",
    originalText: "看雪！！！",
    translatedText: "눈 봐!!!",
    note: "📹 원문 동영상",
    images: [],
    comments: [
      {
        id: 24120501,
        username: "周翊然Tz",
        originalText: "适合配首初雪",
        translatedText: "첫눈 노래랑 잘 어울리겠다",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241206,
    date: "2024-12-01",
    weiboUrl: "https://weibo.com/6224003001/P2RAK9ZiM",
    authorName: "该去碎觉觉辣",
    authorAvatar: "🥣",
    originalText: "在杭州吃的披着辣椒的\u201c甜品\u201d",
    translatedText: "항저우에서 먹은 고추 옷 입은 '디저트'",
    note: "📹 원문 동영상",
    images: [],
    comments: [
      {
        id: 24120601,
        username: "周翊然Tz",
        originalText: "认真看了半天甜品在哪",
        translatedText: "디저트가 어디 있는지 한참 진지하게 찾았네",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241207,
    date: "2024-12-01",
    weiboUrl: "https://weibo.com/7808687821/P2RB4dncA",
    authorName: "丸蛋鱼粥o",
    authorAvatar: "🥣",
    originalText: "能叫法拉利帮我写作业吗",
    translatedText: "페라리한테 내 숙제 좀 대신 해달라고 하면 안 될까?",
    images: [],
    comments: [
      {
        id: 24120701,
        username: "周翊然Tz",
        originalText: "法拉利：这题超纲了",
        translatedText: "페라리: 이 문제는 내 능력 밖이야",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241208,
    date: "2024-12-01",
    weiboUrl: "https://weibo.com/5683706046/P2RBwb29r",
    authorName: "椰耶逗",
    authorAvatar: "🥣",
    originalText: "翻我的话今天就给我的狗加餐",
    translatedText: "내 글에 댓글 달아주면 오늘 우리 강아지 특식 쏜다",
    note: "📹 원문 동영상",
    images: [],
    comments: [
      {
        id: 24120801,
        username: "周翊然Tz",
        originalText: "让孩子多吃点",
        translatedText: "맛있는 거 많이 먹여줘",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241209,
    date: "2024-12-01",
    weiboUrl: "https://weibo.com/7497610356/P2RBKwYou",
    authorName: "吃一口芝士z",
    authorAvatar: "🥣",
    originalText: "放几个保镖在这 看谁敢不回我 👦🏻👦🏻👦🏼👦👦🏼👦🏻👦👦🏼👦🏽👦🏾 👔👔👔👔👔👔👔👔👔👔 👖👖👖👖👖👖👖👖👖👖",
    translatedText: "여기 경호원 몇 명 세워둘 테니까 감히 누가 답글 안 다나 보자 👦🏻👦🏻👦🏼👦👦🏼👦🏻👦👦🏼👦🏽👦🏾 👔👔👔👔👔👔👔👔👔👔 👖👖👖👖👖👖👖👖👖👖",
    images: [],
    comments: [
      {
        id: 24120900,
        username: "吃瓜罗伯特",
        originalText: "小姐姐霸气侧漏，周翊然看到一定会回复的！加油～",
        translatedText: "언니 포스 대박이다, 周翊然이 보면 분명 답장할 거야! 파이팅~",
        images: [],
        actorReply: {
          originalText: "[疑问]",
          translatedText: "[疑问]"
        }
      }
    ]
  },
  {
    id: 241210,
    date: "2024-12-01",
    weiboUrl: "https://weibo.com/6577547727/P2RDTcz57",
    authorName: "哭泣鱼丸丸丸-",
    authorAvatar: "🥣",
    originalText: "来昆明看海鸥啊！小然！（大师！请指点拍照技术[可怜]）",
    translatedText: "쿤밍에 갈매기 보러 와! 익연아! (사진 거장님! 촬영 팁 좀 전수해 주세요[可怜])",
    images: ["images/2412-sea.jpg"],
    comments: [
      {
        id: 24121001,
        username: "周翊然Tz",
        originalText: "风景好看就已经很出片了",
        translatedText: "풍경이 예쁘면 사진은 그냥 잘 나오게 되어 있어",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241211,
    date: "2024-12-01",
    weiboUrl: "https://weibo.com/7910709947/P2REGsQfm",
    authorName: "小狗开车追日落",
    authorAvatar: "🥣",
    originalText: "新歌想好名字了吗[并不简单][并不简单][并不简单]",
    translatedText: "신곡 이름 정했어?[并不简单][并不简单][并不简单]",
    images: [],
    comments: [
      {
        id: 24121101,
        username: "周翊然Tz",
        originalText: "还没",
        translatedText: "아직",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241212,
    date: "2024-12-01",
    weiboUrl: "https://weibo.com/7782270164/P2RFbdX6n",
    authorName: "翊是你Tz",
    authorAvatar: "🥣",
    originalText: "诺 我家孩子 怎么样！",
    translatedText: "짜잔 우리 애 어때!",
    images: ["images/2412-dog3.jpg"],
    comments: [
      {
        id: 24121201,
        username: "周翊然Tz",
        originalText: "英俊",
        translatedText: "훤칠하네",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241213,
    date: "2024-12-01",
    weiboUrl: "https://weibo.com/7836211859/P2RFpyLFX",
    authorName: "HeWei何薇",
    authorAvatar: "🥣",
    originalText: "我的状态就跟我家猫一样",
    translatedText: "내 상태 딱 우리 집 고양이 상태임",
    note: "📹 원문 동영상",
    images: [],
    comments: [
      {
        id: 24121301,
        username: "周翊然Tz",
        originalText: "很松弛",
        translatedText: "아주 여유롭고 좋구만",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241214,
    date: "2024-12-01",
    weiboUrl: "https://weibo.com/7931103121/P2RFm9WGU",
    authorName: "困困邶",
    authorAvatar: "🥣",
    originalText: "小猫咪，可以帮他取个名字嘛，马上模拟考了可以祝我超常发挥嘛然然[抱一抱]",
    translatedText: "아기 고양이 이름 좀 지어줄 수 있어? 곧 모의고사인데 대박 나라고 빌어줘 익연아[抱一抱]",
    images: ["images/2412-cat3.jpg"],
    comments: [
      {
        id: 24121401,
        username: "周翊然Tz",
        originalText: "就叫 超常发挥 吧💪",
        translatedText: "그럼 이름을'대박'이로 하자💪",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241215,
    date: "2024-12-01",
    weiboUrl: "https://weibo.com/5845364559/P2RFjdJRZ",
    authorName: "别动田梨涡_",
    authorAvatar: "🥣",
    originalText: "想念夏天的海边🥹昨天找到了一组两个月前洗出来的胶片！分享给你～",
    translatedText: "여름 바다가 그리워🥹 어제 두 달 전에 인화한 필름 사진 찾았어! 너한테 공유할게~",
    images: ["images/2412-sea5.jpg"],
    comments: [
      {
        id: 24121501,
        username: "周翊然Tz",
        originalText: "拍得真好",
        translatedText: "진짜 잘 찍었다",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241216,
    date: "2024-12-01",
    weiboUrl: "https://weibo.com/7290255850/P2RGfFLob",
    authorName: "菠萝小薇耶",
    authorAvatar: "🥣",
    originalText: "求一下哈尔滨的美食！跟穿搭",
    translatedText: "하얼빈 맛집이랑 코디 추천 좀 해줘!",
    images: [],
    comments: [
      {
        id: 24121601,
        username: "周翊然Tz",
        originalText: "铁锅炖！",
        translatedText: "티에궈둔!!",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241217,
    date: "2024-12-01",
    weiboUrl: "https://weibo.com/6209313530/P2RGepqF2",
    authorName: "Nunununu努",
    authorAvatar: "🥣",
    originalText: "树叶 牛不牛",
    translatedText: "나뭇잎으로 만든 건데, 쩔지않아?",
    note: "📹 원문 동영상",
    images: [],
    comments: [
      {
        id: 24121701,
        username: "周翊然Tz",
        originalText: "怎么做的",
        translatedText: "어떻게 만든 거야",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241218,
    date: "2024-12-01",
    weiboUrl: "https://weibo.com/7787755171/P2RGGCuh4",
    authorName: "长夏易燃-",
    authorAvatar: "🥣",
    originalText: "看今天北京的晚霞！",
    translatedText: "오늘 베이징 노을 봐!",
    note: "📹 원문 동영상",
    images: [],
    comments: [
      {
        id: 24121801,
        username: "周翊然Tz",
        originalText: "好漂亮",
        translatedText: "너무 예쁘다",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241219,
    date: "2024-12-01",
    weiboUrl: "https://weibo.com/5994946077/P2RHuB1hm",
    originalText: "一会得继续拍戏 下次见",
    translatedText: "이따가 또 촬영하러 가야 해 다음에 봐",
    images: [],
    comments: []
  },
  {
    id: 241220,
    date: "2024-12-02",
    weiboUrl: "https://weibo.com/5994946077/P32K0hKbK",
    originalText: "来了 转场中",
    translatedText: "등장 장소 이동 중",
    images: [],
    comments: [
      {
        id: 24122001,
        username: "周翊然Tz",
        originalText: "吃晚饭了不",
        translatedText: "다들 저녁 먹었어?",
        images: [],
        actorReply: null
      },
      {
        id: 24122002,
        username: "周翊然Tz",
        originalText: "我怎么发现每天都人在吃火锅",
        translatedText: "근데 왜 맨날 다들 훠궈 먹고 있는 것 같지",
        images: [],
        actorReply: null
      },
      {
        id: 24122003,
        username: "周翊然Tz",
        originalText: "想去看蜡笔小新的电影 但没时间 有看过的人吗 好看吗？？？",
        translatedText: "짱구 극장판 보고 싶은데 시간이 없어 본 사람? 재밌어??",
        images: [],
        actorReply: null
      },
      {
        id: 24122004,
        username: "周翊然Tz",
        originalText: "我溜了 各位 到现场了[收到]",
        translatedText: "나 이만 가볼게 얘들아 나 현장 도착했어[收到]",
        images: [],
        actorReply: null
      },
      {
        id: 24122005,
        username: "粥里有什么_",
        originalText: "吃火锅",
        translatedText: "훠궈 먹는 중",
        images: ["images/2412-hotpot.jpg"],
        actorReply: {
          originalText: "舒服了 我猜那个是鹅肠",
          translatedText: "아 좋겠다 저거 거위 창자 맞지?"
        }
      },
      {
        id: 24122007,
        username: "草莓云朵kitty_",
        originalText: "然然，我明天生日，可以祝我生日快乐吗[爱慕]",
        translatedText: "익연아, 나 내일 생일인데 생일 축하해줄 수 있어?[爱慕]",
        images: ["images/2412-birthday1.jpg"],
        actorReply: {
          originalText: "生日快乐！！",
          translatedText: "생일 축하해!!"
        }
      },
      {
        id: 24122009,
        username: "你跑到哪里去啦",
        originalText: "宝宝你快看猪",
        translatedText: "익연아 빨리 돼지 좀 봐봐",
        images: ["images/2412-pig.jpg"],
        actorReply: {
          originalText: "哈哈哈哈哈哈哈哈哈哈哈哈",
          translatedText: "ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ"
        }
      },
      {
        id: 24122011,
        username: "-荔枝不荔枝-",
        originalText: "喝！",
        translatedText: "마셔!",
        images: ["images/2412-drink.jpg"],
        actorReply: {
          originalText: "春日部防卫队[doge]",
          translatedText: "떡잎마을 방위대[doge]"
        }
      },
      {
        id: 24122013,
        username: "粥里有什么_",
        originalText: "你看 猫咪台阶补色了！",
        translatedText: "봐봐 고양이 계단 색칠 다시 다 했어!",
        images: ["images/2412-cat-stairs.jpg"],
        actorReply: {
          originalText: "熟悉的地方",
          translatedText: "익숙한 곳이네"
        }
      },
      {
        id: 24122015,
        username: "是我回来啦-",
        originalText: "你看我刷到的老抽色的狗 他们说老抽色的狗心眼子多",
        translatedText: "내가 탐라에서 본 진간장색 강아지 좀 봐 다들 그러던데 진간장색 강아지가 꾀가 많대",
        images: ["images/2412-dark-dog.jpg"],
        actorReply: {
          originalText: "眼神看着 很睿智啊",
          translatedText: "눈빛이 되게 영특해 보이는데?"
        }
      },
      {
        id: 24122017,
        username: "甜桃兔丸丸x",
        originalText: "宝宝今天是我的生日！！！",
        translatedText: "익연아 오늘 내 생일이야!!!",
        images: ["images/2412-birthday2.jpg"],
        actorReply: {
          originalText: "生日快乐！",
          translatedText: "생일 축하해!"
        }
      },
      {
        id: 24122019,
        username: "樱花味的cherry",
        originalText: "你就说可爱不？。我家狗心都要碎了哈",
        translatedText: "귀여워 안 귀여워 우리 집 강아지 삐져서 섭섭해죽겠어ㅋ",
        images: ["images/2412-cute-dog.jpg"],
        actorReply: {
          originalText: "[doge]",
          translatedText: "[doge]"
        }
      },
      {
        id: 24122021,
        username: "熏鸭拱远点",
        originalText: "你晚上吃的啥",
        translatedText: "넌 저녁에 뭐 먹었어?",
        images: [],
        actorReply: {
          originalText: "一个鸡腿",
          translatedText: "닭다리 하나"
        }
      },
      {
        id: 24122023,
        username: "Lupaovran",
        originalText: "我这特别冷，你那呢[顶]",
        translatedText: "여기는 완전 추운데 거긴 어때[顶]",
        images: ["images/2412-cold.jpg"],
        actorReply: {
          originalText: "开空调啊",
          translatedText: "히터 틀고있어"
        }
      },
      {
        id: 24122025,
        username: "祝你米奇妙妙屋y",
        originalText: "在吃新疆炒米粉",
        translatedText: "신장 볶음 쌀국수 먹는 중",
        images: [],
        actorReply: {
          originalText: "新疆手抓饭 无敌",
          translatedText: "신장은 손으로 먹는 볶음밥이 최곤데"
        }
      },
      {
        id: 24122027,
        username: "阿泊切爹翊",
        originalText: "老公 你倒是回我啊 别人都以为我单相思",
        translatedText: "익연아 답장 좀 해줘 남들이 보면 내가 짝사랑하는 줄 알겠어",
        images: [],
        actorReply: {
          originalText: "好的",
          translatedText: "알았어"
        }
      },
      {
        id: 24122029,
        username: "Yirrrraan",
        originalText: "宝宝这是你的小巧思对吗[兔子][爱慕]",
        translatedText: "익연아 이거 너가 생각한 아이디어 맞지?[兔子][爱慕]",
        images: ["images/2412-craft.jpg"],
        actorReply: {
          originalText: "✌️",
          translatedText: "✌️"
        }
      },
      {
        id: 24122031,
        username: "1iiouo",
        originalText: "吃饭了吗",
        translatedText: "밥 먹었어?",
        images: ["images/2412-meal.jpg"],
        actorReply: {
          originalText: "吃过了",
          translatedText: "먹었어"
        }
      },
      {
        id: 24122033,
        username: "睡个什么觉好呢Tz",
        originalText: "拍的怎么样",
        translatedText: "사진 찍은 거 어때",
        images: ["images/2412-photo1.jpg"],
        actorReply: {
          originalText: "很美好 哈哈哈",
          translatedText: "너무 좋다 ㅋㅋㅋ"
        }
      },
      {
        id: 24122035,
        username: "着急过冬",
        originalText: "能不能回一下着急过冬？",
        translatedText: "着急过冬한테 답장 좀 해줄 수 있어?",
        images: [],
        actorReply: {
          originalText: "咋了",
          translatedText: "왜?"
        }
      },
      {
        id: 24122037,
        username: "永远喜欢真诚小狗",
        originalText: "宝宝我今天换了新的科室 手术室 后面两周简单的手术要我上台做器护 有点紧张",
        translatedText: "익연아 나 오늘 새로운 과로 옮겼어 수술실인데 앞으로 2주 동안 간단한 수술 기구 간호사로 서야 하는데 좀 긴장돼",
        images: [],
        actorReply: {
          originalText: "放平心态 加油！",
          translatedText: "마음 편하게 먹어 화이팅!"
        }
      },
      {
        id: 24122039,
        username: "翊只臭臭Tz",
        originalText: "和你的合照 怎么样！",
        translatedText: "너랑 같이 찍은 합성사진 어때!",
        images: [],
        actorReply: {
          originalText: "\u201c✌️\u201d差点意思",
          translatedText: "'✌️' 포즈가 약간 아쉽다"
        }
      },
      {
        id: 24122041,
        username: "cc小鱼卷饼",
        originalText: "哥哥你怎么又来了……",
        translatedText: "오빠 왜 또 왔어……",
        images: [],
        actorReply: {
          originalText: "感觉你不太想让我来",
          translatedText: "너 내가 오는 거 별로 안 반가워하는 것 같다?"
        }
      },
      {
        id: 24122043,
        username: "偷吃一口饭饭",
        originalText: "好久没见茅台了！你是不是把他藏起来了",
        translatedText: "마오타이 진짜 오랜만이야! 너 어디 숨겨둔 거 아니지?",
        images: [],
        actorReply: {
          originalText: "换毛呢",
          translatedText: "털갈이 중이라서 그래"
        }
      },
      {
        id: 24122045,
        username: "Mercuree_",
        originalText: "明天考科一已经开始紧张了",
        translatedText: "내일 1종 시험인데 벌써부터 떨린다",
        images: [],
        actorReply: {
          originalText: "轻轻松松[doge]",
          translatedText: "껌이지 [doge]"
        }
      },
      {
        id: 24122047,
        username: "给我萌兔了",
        originalText: "没有好吃的但是有减脂餐",
        translatedText: "맛있는 건 없지만 다이어트식은 있어",
        images: ["images/2412-diet-meal.jpg"],
        actorReply: {
          originalText: "鸡胸肉吗",
          translatedText: "닭가슴살이야?"
        }
      },
      {
        id: 24122049,
        username: "翊只猪猪崽Tz",
        originalText: "然然猜猜这是啥",
        translatedText: "익연아 이게 뭔지 맞혀봐",
        images: ["images/2412-guess1.jpg"],
        actorReply: {
          originalText: "热红酒？",
          translatedText: "뱅쇼인가?"
        }
      },
      {
        id: 24122051,
        username: "幸运梨梨梨",
        originalText: "宝宝！我朋友都说是狗 你猜猜看！[打call]",
        translatedText: "자기야! 친구들은 다 강아지라는데 너가 맞혀봐![打call]",
        images: ["images/2412-guess2.jpg"],
        actorReply: {
          originalText: "🐱",
          translatedText: "🐱"
        }
      },
      {
        id: 24122053,
        username: "交换涟漪io",
        originalText: "最近在听什么歌？[爱慕]",
        translatedText: "요즘 무슨 노래 들어?[爱慕]",
        images: [],
        actorReply: {
          originalText: "瘦子很好听",
          translatedText: "瘦子 노래 되게 좋아"
        }
      },
      {
        id: 24122055,
        username: "珺林天下Y",
        originalText: "我不敢开车上路 怎么办",
        translatedText: "무서워서 도로 운전을 못 하겠어 어떡하지",
        images: [],
        actorReply: {
          originalText: "让副驾驶坐个老司机先带带你",
          translatedText: "조수석에 베테랑 태우고 먼저 좀 배워봐"
        }
      },
      {
        id: 24122057,
        username: "珍珍大王_xyt",
        originalText: "然然今日份的横店冻人吗？",
        translatedText: "익연아 오늘 헝디엔 사람 얼 정도로 추워?",
        images: [],
        actorReply: {
          originalText: "早晚温差很大",
          translatedText: "아침저녁 일교차가 심해"
        }
      },
      {
        id: 24122059,
        username: "唉又萌混过关zZ",
        originalText: "看我今天新学的包扎",
        translatedText: "오늘 새로 배운 붕대 감기법",
        images: ["images/2412-bandage.jpg"],
        actorReply: {
          originalText: "主打一个保暖",
          translatedText: "보온이 핵심이네"
        }
      },
      {
        id: 24122061,
        username: "24号赛车手_",
        originalText: "敢不敢保佑我",
        translatedText: "나 좀 지켜줄 수 있어?",
        images: ["images/2412-racing1.jpg"],
        actorReply: {
          originalText: "哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈 保佑你",
          translatedText: "ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ 지켜줄게"
        }
      },
      {
        id: 24122063,
        username: "偷吃一口饭饭",
        originalText: "哥哥 小赛车好玩不 什么时候开赛车想你开车了！！",
        translatedText: "오빠 레이싱 재밌어? 언제 레이싱카 몰 거야 오빠 운전하는 거 보고 싶어!!",
        images: ["images/2412-racing2.jpg"],
        actorReply: {
          originalText: "[doge]",
          translatedText: "[doge]"
        }
      },
      {
        id: 24122065,
        username: "香菜胃99",
        originalText: "刷到一只小可爱",
        translatedText: "귀여운 애 하나 발견",
        images: ["images/2412-cute-find.jpg"],
        actorReply: {
          originalText: "…..",
          translatedText: "….."
        }
      }
    ]
  },
  {
    id: 241221,
    date: "2024-12-19",
    weiboUrl: "https://weibo.com/5994946077/P5BSumNud",
    originalText: "上来看看",
    translatedText: "잠깐 보러 왔어",
    images: [],
    comments: [
      {
        id: 24122101,
        username: "周翊然Tz",
        originalText: "先下线了 下次见",
        translatedText: "먼저 갈게 담에 봐",
        images: [],
        actorReply: null
      },
      {
        id: 24122102,
        username: "烦恼豆沙包z",
        originalText: "今中午吃的火锅[爱慕]",
        translatedText: "오늘 점심에 먹은 훠궈[爱慕]",
        images: ["images/2412-hotpot2.jpg"],
        actorReply: {
          originalText: "每天都有人在吃火锅",
          translatedText: "맨날 누군가는 훠궈를 꼭 먹고 있네"
        }
      },
      {
        id: 24122104,
        username: "小盈油炸地球",
        originalText: "想吃吗宝宝",
        translatedText: "먹고 싶지 익연아?",
        images: ["images/2412-craving.jpg"],
        actorReply: {
          originalText: "还有烤肉",
          translatedText: "고기도 있네"
        }
      }
    ]
  },
  {
    id: 241222,
    date: "2024-12-19",
    weiboUrl: "",
    authorName: "1ikeey",
    authorAvatar: "🥣",
    originalText: "下班了吗老铁",
    translatedText: "퇴근했나 브로",
    images: [],
    comments: [
      {
        id: 24122201,
        username: "周翊然Tz",
        originalText: "还没",
        translatedText: "아직",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241223,
    date: "2024-12-19",
    weiboUrl: "",
    authorName: "耶耶不拿铁Oo",
    authorAvatar: "🥣",
    originalText: "我的新头盔怎么样[开学季]",
    translatedText: "내 새 헬멧 어때[开学季]",
    images: ["images/2412-helmet.jpg"],
    comments: [
      {
        id: 24122301,
        username: "周翊然Tz",
        originalText: "酷👍🏻",
        translatedText: "간지👍🏻",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241224,
    date: "2024-12-19",
    weiboUrl: "",
    authorName: "Heliotrope1122",
    authorAvatar: "🥣",
    originalText: "我今天生日，可以祝我生日快乐吗？然然",
    translatedText: "나 오늘 생일인데 생일 축하해줄 수 있어? 익연아",
    images: [],
    comments: [
      {
        id: 24122401,
        username: "周翊然Tz",
        originalText: "生日快乐",
        translatedText: "생일 축하해",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241225,
    date: "2024-12-19",
    weiboUrl: "",
    authorName: "洋芋不吃土豆_",
    authorAvatar: "🥣",
    originalText: "正在做陶艺得我 也腾出一只手来看看",
    translatedText: "도예 하다가 손 한 쪽 겨우 빼서 왔어",
    note: "📹 원문 동영상",
    images: [],
    comments: [
      {
        id: 24122501,
        username: "周翊然Tz",
        originalText: "这是水杯吗",
        translatedText: "이거 물컵이야?",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241226,
    date: "2024-12-19",
    weiboUrl: "",
    authorName: "小猫椰绒",
    authorAvatar: "🥣",
    originalText: "我到史莱克学院了，怎么没看见你啊三哥[疑问]",
    translatedText: "슈렉학원 왔는데 왜 셋째 오빠가 안 보이지[疑问]",
    images: ["images/2412-slake.jpg"],
    comments: [
      {
        id: 24122601,
        username: "周翊然Tz",
        originalText: "三哥最近外出游历中",
        translatedText: "셋째 오빠 요즘 수련 중이야",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241227,
    date: "2024-12-22",
    weiboUrl: "",
    originalText: "化妆间的网速不太好",
    translatedText: "분장실 와이파이가 좀 안 좋네",
    images: [],
    comments: [
      {
        id: 24122701,
        username: "周翊然Tz",
        originalText: "先下线了 下次见",
        translatedText: "이만 나볼게 담에 보자",
        images: [],
        actorReply: null
      },
      {
        id: 24122702,
        username: "综上所述你最萌",
        originalText: "还没收工吗？！！",
        translatedText: "아직 퇴근 안 했어?!!",
        images: [],
        actorReply: {
          originalText: "刚上班",
          translatedText: "이제 막 출근했어"
        }
      },
      {
        id: 24122704,
        username: "忙内专家",
        originalText: "小然给你看我的小猫",
        translatedText: "익연아 내 고영이 보여줄게",
        images: ["images/2412-cat4.jpg"],
        actorReply: {
          originalText: "帽子不错",
          translatedText: "모자 예쁘다"
        }
      },
      {
        id: 24122706,
        username: "走啊摘星星",
        originalText: "过得这是哪国时间[躺平]",
        translatedText: "대체 어느 나라 시차로 사는 거야[躺平]",
        images: [],
        actorReply: {
          originalText: "横国",
          translatedText: "헝디엔국"
        }
      },
      {
        id: 24122708,
        username: "椿雨遇翊",
        originalText: "猜猜什么味儿的？",
        translatedText: "무슨 맛인지 맞혀볼래?",
        images: ["images/2412-guess3.jpg"],
        actorReply: {
          originalText: "甜的",
          translatedText: "단맛"
        }
      }
    ]
  },
  {
    id: 241228,
    date: "2024-12-22",
    weiboUrl: "",
    authorName: "天气骤然小雨-",
    authorAvatar: "🥣",
    originalText: "To：12月的小然\nHello啊 然然 不知什么时候可以见到你，所以写下了这一封电子信\n初三后时间过得很快，每天两点一线，甚至早上起来上学都能看到月亮，晚上还有月亮陪伴着回家 每当我看向月亮时，我常会想到\"你看到会不会是和我同一时间段的月亮\"所以我想把我拍到的月亮分享给你\n每周休息时，总喜欢呆在爷爷的小菜园里，就连小猫也向往着那唯一的一缕阳光（悄悄告诉你，小猫的鼻子中间有一块黑点，所以我们经常喊他喵桑）除此之外，我还会靠零食来鼓励每一天的自己，每天上学前总会用此安慰自己 不知道然然每天坚持下去的动力会不会是我们甜粥呢？\n12月的突击考也终于过去了，虽然着急习惯了学校突击的测试，但每次还是会很紧张，对了这次的考号竟然是你的生日！！！这次考试后我听见老师口中最常说的一句话就是不要放弃自己，其实我也很畏惧前方的位置可如果我不向前，那我就不能跟上你的脚步所以接下来我可能要打一场硬仗啦\n最近朋友推荐了我一本书，感觉真的非常的治愈，想分享给你 有没有觉得里面的主人公小狗长得非常像法拉利呀 就像书上所说的\n\"未来也许依旧不那么清晰\n但这些小而具体的理由正是照亮人生脚步的微弱光点\n指引着我们继续往前走\"\n那么接下来就祝你祝我 事业学业顺顺利利 天天开心 平安喜乐",
    translatedText: "To: 12월의 익연이에게\nHello 익연아, 언제 만날 수 있을지 몰라서 이렇게 모바일 편지를 적어봐\n중학교 3학년이 되고 나서는 시간이 너무 빠르게 흘러가는 것 같아. 매일 집과 학교만 반복하는 일상이고, 아침 일찍 등교할 때도 달을 보고, 밤에 집에 돌아갈 때도 달이 날 지켜봐 주거든. 달을 바라볼 때마다 '너도 혹시 지금 나와 같은 시각의 달을 보고 있을까?' 하는 생각이 들어서 내가 직접 찍은 달 사진을 보여주고 싶었어.\n주말에 쉴 때면 늘 할아버지의 작은 텃밭에 가 있곤 해. 고양이조차도 비치는 한 줄기 햇살을 따스하게 쬐는 걸 좋아하더라고 (비밀인데 알려주자면, 그 고양이는 코 한가운데에 까만 점이 있어서 우리가 늘 '냥상'이라고 불러!). 그리고 나는 매일 간식을 먹으면서 스스로를 응원하곤 해. 아침에 등교하기 전에 늘 그렇게 마음을 달래지. 익연이가 매일 힘을 내는 원동력도 혹시 우리 '톈저우(甜粥)' 팬들일까?\n12월 쪽지시험도 드디어 끝났어. 학교에서 불시에 보는 시험에 익숙해졌다고 생각하면서도 매번 긴장되는 건 어쩔 수 없더라. 그런데 이번 시험 수험번호가 글쎄 너 생일 번호였어!!! 이번 시험 끝나고 선생님들께서 제일 많이 하신 말씀이 '절대 포기하지 마라'였어. 사실 나도 앞에 놓인 내 미래나 목표가 겁나기도 하지만, 나아가지 않으면 너의 발자취를 따라갈 수 없잖아. 그래서 앞으로 진짜 단단히 마음먹고 힘내보려고 해!\n최근에 친구가 책을 한 권 추천해 줬는데 정말 힐링 되어서 너한테도 나누고 싶었어. 거기 나오는 주인공 강아지가 페라리랑 진짜 닮았다고 느끼지 않았어? 책에 이런 문구가 있더라고.\n'미래가 여전히 불투명할지라도,\n이 소소하고 구체적인 이유들이야말로 인생의 길을 비추는 희미한 희망의 빛이 되어\n우리를 계속 나아가게 해준다.'\n그럼 너도, 나도 하는 일과 학업 모두 잘 풀리고, 매일 웃을 일만 가득하며 평안하길 바랄게",
    note: "📹 원문 동영상",
    images: [],
    comments: [
      {
        id: 24122801,
        username: "周翊然Tz",
        originalText: "顺顺利利 天天开心",
        translatedText: "항상 모든 일이 다 잘 풀리고 매일 행복했으면 좋겠다",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241229,
    date: "2024-12-22",
    weiboUrl: "",
    authorName: "一个努力干活还不磨人的小妖精",
    authorAvatar: "🥣",
    originalText: "我染头发的时候理发店的猫一直陪着我 超可爱 你也看看",
    translatedText: "나 염색하는 동안 미용실 고양이가 옆에 착 붙어서 계속 같이 있어줬어 완전 귀여워 너도 봐봐",
    note: "📹 원문 동영상",
    images: [],
    comments: [
      {
        id: 24122901,
        username: "周翊然Tz",
        originalText: "这么可爱的猫有两只",
        translatedText: "이렇게 귀여운 고양이가 두 마리나 있다니",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241230,
    date: "2024-12-22",
    weiboUrl: "",
    authorName: "我真的只想开机车啊",
    authorAvatar: "🥣",
    originalText: "后天考驾照了然然能不能祝我顺利考过",
    translatedText: "익연아 모레 운전면허 시험인데 합격 응원 좀 많이 해줄 수 있어?",
    images: ["images/2412-license.jpg"],
    comments: [
      {
        id: 24123001,
        username: "周翊然Tz",
        originalText: "一定能过",
        translatedText: "무조건 붙을 거야",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241231,
    date: "2024-12-22",
    weiboUrl: "",
    authorName: "吃好睡好天下第一好",
    authorAvatar: "🥣",
    originalText: "看我家小狗的新衣服 好看吗",
    translatedText: "우리 강아지 새 옷 샀는데 봐봐 예쁘지?",
    images: ["images/2412-dogclothes.jpg"],
    comments: [
      {
        id: 24123101,
        username: "周翊然Tz",
        originalText: "梦回东北了",
        translatedText: "순간 동북 지방 온 줄",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241232,
    date: "2024-12-22",
    weiboUrl: "",
    authorName: "超绝然然",
    authorAvatar: "🥣",
    originalText: "看我的架子鼓帅吗？",
    translatedText: "내 드럼 멋있지?",
    images: ["images/2412-drum.jpg"],
    comments: [
      {
        id: 24123201,
        username: "周翊然Tz",
        originalText: "🆒",
        translatedText: "🆒",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241233,
    date: "2024-12-22",
    weiboUrl: "",
    authorName: "温沅Tz_",
    authorAvatar: "🥣",
    originalText: "然然快看我在和你一起变好🥰",
    translatedText: "익연아 나 너 따라서 점점 더 좋아지고 있어🥰",
    images: ["images/2412-together.jpg"],
    comments: [
      {
        id: 24123301,
        username: "周翊然Tz",
        originalText: "好厉害",
        translatedText: "대단하다",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241234,
    date: "2024-12-22",
    weiboUrl: "",
    authorName: "牙不吃香菜",
    authorAvatar: "🥣",
    originalText: "然然看熊猫",
    translatedText: "익연아 판다 봐봐",
    note: "📹 원문 동영상",
    images: [],
    comments: [
      {
        id: 24123400,
        username: "吃瓜罗伯特",
        originalText: "然然和熊猫一样可爱，都是国宝级的存在呢~",
        translatedText: "익연이랑 판다랑 똑같이 귀엽네, 둘 다 국보급 존재야~",
        images: [],
        actorReply: {
          originalText: "[疑问]",
          translatedText: "[疑问]"
        }
      }
    ]
  },
  {
    id: 241235,
    date: "2024-12-22",
    weiboUrl: "",
    authorName: "骤然吐司yr",
    authorAvatar: "🥣",
    originalText: "乌龟大军来袭🐢",
    translatedText: "거북이 대군 습격🐢",
    images: ["images/2412-turtles.jpg"],
    comments: [
      {
        id: 24123501,
        username: "周翊然Tz",
        originalText: "怎么混进一只mini龟",
        translatedText: "미니 거북이 한 마리는 왜 껴있어",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241236,
    date: "2024-12-22",
    weiboUrl: "",
    authorName: "内娱第一甜弟-",
    authorAvatar: "🥣",
    originalText: "法拉利和茅台能这样吗😁",
    translatedText: "페라리랑 마오타이 이래도 되는거야?😁",
    images: ["images/2412-pets2.jpg"],
    comments: [
      {
        id: 24123601,
        username: "周翊然Tz",
        originalText: "不太行",
        translatedText: "이건 좀 아니지",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241237,
    date: "2024-12-22",
    weiboUrl: "",
    authorName: "Nniu又困了",
    authorAvatar: "🥣",
    originalText: "刚拍的铁花✨分享给你",
    translatedText: "방금 찍은 불꽃 사진✨ 공유 해줄게",
    note: "📹 원문 동영상",
    images: [],
    comments: [
      {
        id: 24123701,
        username: "周翊然Tz",
        originalText: "哇",
        translatedText: "우와",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241238,
    date: "2024-12-22",
    weiboUrl: "",
    authorName: "Aaaa甜粥zyr",
    authorAvatar: "🥣",
    originalText: "然然给你分享一个粉色的天",
    translatedText: "익연아 핑크빛 하늘 보여줄게",
    images: ["images/2412-pinksky.jpg"],
    comments: [
      {
        id: 24123801,
        username: "周翊然Tz",
        originalText: "好看",
        translatedText: "예쁘다",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241239,
    date: "2024-12-22",
    weiboUrl: "",
    authorName: "等三十秒月初_",
    authorAvatar: "🥣",
    originalText: "给你看看恐龙化石",
    translatedText: "공룡 화석 좀 구경시켜줄까",
    images: ["images/2412-fossil.jpg"],
    comments: [
      {
        id: 24123901,
        username: "周翊然Tz",
        originalText: "好霸气",
        translatedText: "완전 포스있다",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241240,
    date: "2024-12-22",
    weiboUrl: "",
    authorName: "Starry--YH",
    authorAvatar: "🥣",
    originalText: "快看树🎄也穿上了新衣服啦！！！",
    translatedText: "나무🎄도 새 옷 입었어 봐봐!!!",
    images: ["images/2412-tree.jpg"],
    comments: [
      {
        id: 24124001,
        username: "周翊然Tz",
        originalText: "穿搭品味不错",
        translatedText: "패션 센스 괜찮네",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241241,
    date: "2024-12-22",
    weiboUrl: "",
    authorName: "予翊一笺诗",
    authorAvatar: "🥣",
    originalText: "让法拉利和茅台好好学习一下",
    translatedText: "페라리랑 마오타이 열심히 공부시켜야겠다",
    images: ["images/2412-study.jpg"],
    comments: [
      {
        id: 24124101,
        username: "周翊然Tz",
        originalText: "法拉利可以 茅台算了",
        translatedText: "페라리는 되는데 마오타이는 글렀어",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241242,
    date: "2024-12-22",
    weiboUrl: "",
    authorName: "随你ran",
    authorAvatar: "🥣",
    originalText: "然然快看！",
    translatedText: "익연아 빨리!",
    images: ["images/2412-look2.jpg"],
    comments: [
      {
        id: 24124201,
        username: "周翊然Tz",
        originalText: "不冷吗",
        translatedText: "안 추워?",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241243,
    date: "2024-12-22",
    weiboUrl: "",
    authorName: "吃不吃花生酱呢",
    authorAvatar: "🥣",
    originalText: "然然 给你看我在西湖旁边看到的松鼠🐿️",
    translatedText: "익연아 서호 옆에서 본 다람쥐 보여줄게🐿️",
    images: ["images/2412-squirrel.jpg"],
    comments: [
      {
        id: 24124300,
        username: "吃瓜罗伯特",
        originalText: "好可爱的小松鼠呀，然然看到了一定会很开心的！",
        translatedText: "완전 귀여운 다람쥐네, 익연이가 보면 진짜 좋아할 거야!",
        images: [],
        actorReply: {
          originalText: "罗伯特怎么总比我快",
          translatedText: "로버트는 왜 맨날 나보다 빠른거지"
        }
      },
      {
        id: 24124302,
        username: "周翊然Tz",
        originalText: "发型有点酷",
        translatedText: "헤어스타일 좀 멋있네",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241244,
    date: "2024-12-22",
    weiboUrl: "",
    authorName: "_33three_",
    authorAvatar: "🥣",
    originalText: "我是薯条大王",
    translatedText: "나는 감자튀김 대왕이다",
    images: ["images/2412-fries.jpg"],
    comments: [
      {
        id: 24124401,
        username: "周翊然Tz",
        originalText: "你好薯条大王",
        translatedText: "안녕하세요 감자튀김 대왕님",
        images: [],
        actorReply: null
      }
    ]
  },
  {
    id: 241245,
    date: "2024-12-31",
    weiboUrl: "https://weibo.com/5994946077/P7s9wmPHR",
    originalText: "hello",
    translatedText: "hello",
    images: [],
    comments: [
      {
        id: 24124501,
        username: "周翊然Tz",
        originalText: "新的一年 希望大家在新的一年可以一切顺利 身体健康 财源广进 开开心心\n一起告别24 迎接25 新年快乐！！！[2025][2025][2025]",
        translatedText: "새해에는 다들 모든 일이 잘 풀리고, 건강하고, 부자 되고, 늘 행복하길\n같이 24년 잘 마무리하고 25년 맞이하자 새해 복 많이 받아!!![2025][2025][2025]",
        images: [],
        actorReply: null
      },
      {
        id: 24124502,
        username: "周翊然Tz",
        originalText: "大家在干嘛呢",
        translatedText: "다들 뭐 하고 있어",
        images: [],
        actorReply: null
      },
      {
        id: 24124503,
        username: "周翊然Tz",
        originalText: "本来今天想直播的 但实在没时间[裂开]",
        translatedText: "원래 오늘 라방 하려고 했는데 진짜 시간이 없어[裂开]",
        images: [],
        actorReply: null
      },
      {
        id: 24124504,
        username: "周翊然Tz",
        originalText: "我先走了 大家玩的开心！ 新年快乐！！ 等等记得刷我微博[收到]",
        translatedText: "나 먼저 갈게 재밌게 놀아! 새해 복 많이 받고!! 이따 내 웨이보 꼭 확인해[收到]",
        images: [],
        actorReply: null
      },
      {
        id: 24124505,
        username: "周翊然Tz",
        originalText: "等戏空隙[收到]",
        translatedText: "촬영 쉬는 시간[收到]",
        images: [],
        actorReply: null
      },
      {
        id: 24124506,
        username: "周翊然Tz",
        originalText: "抠图的那位好人 请记得发一下原图",
        translatedText: "누끼 따준 천사분, 원본 사진 좀 공유해주세요",
        images: [],
        actorReply: null
      },
      {
        id: 24124507,
        username: "樱花味的cherry",
        originalText: "这个头像？什么东西",
        translatedText: "이거 프사? 이게 뭐야",
        images: ["images/2412-cutout.jpg"],
        actorReply: {
          originalText: "咋了",
          translatedText: "뭐가"
        }
      },
      {
        id: 24124509,
        username: "Mennnxg",
        originalText: "许愿明年多见面！",
        translatedText: "내년엔 더 자주 만나게 해달라고 빌어야지!",
        images: ["images/2412-wish.jpg"],
        actorReply: {
          originalText: "一定",
          translatedText: "꼭이야"
        }
      },
      {
        id: 24124511,
        username: "粥里有什么_",
        originalText: "看重庆的跨年气球🎈",
        translatedText: "충칭 카운트다운 풍선 봐🎈",
        images: ["images/2412-balloon.jpg"],
        actorReply: {
          originalText: "很美 但要注意安全",
          translatedText: "예쁘다 근데 다치지 않게 조심해"
        }
      },
      {
        id: 24124513,
        username: "落日屿橘",
        originalText: "老公派你帮我守宿舍了！我放假啦哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈哈",
        translatedText: "자기야 나 대신 기숙사 좀 지켜줘! 나 방학이다 ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ",
        images: ["images/2412-dorm.jpg"],
        actorReply: {
          originalText: "6",
          translatedText: "6"
        }
      },
      {
        id: 24124515,
        username: "是我的小土豆-Tz",
        originalText: "我要吃吐了老公",
        translatedText: "나 토할 때까지 먹었어 자기야",
        images: ["images/2412-full.jpg"],
        actorReply: {
          originalText: "那你别吃了",
          translatedText: "그럼 그만 먹어"
        }
      },
      {
        id: 24124517,
        username: "吃翊口脆薯饼_",
        originalText: "小哥哥你为什么不好好抠图！！",
        translatedText: "오빠 누끼 좀 성의 있게 따봐!!",
        images: ["images/2412-cutout.jpg"],
        actorReply: {
          originalText: "那你倒是发个原图啊！！",
          translatedText: "그럼 원본 사진 좀 올려주든가!!"
        }
      },
      {
        id: 24124519,
        username: "那又咋了0_o",
        originalText: "下次我教你扣图吧",
        translatedText: "다음에 내가 누끼 따는 법 알려줄게",
        images: ["images/2412-cutout.jpg"],
        actorReply: {
          originalText: "确实比我的好 可以发个原图吗",
          translatedText: "확실히 내 것보다 낫네 원본 좀 올려줄 수 있어?"
        }
      },
      {
        id: 24124521,
        username: "美少女壮士_lucky",
        originalText: "哥哥哥哥哥哥哥",
        translatedText: "오빠오빠오빠오빠오빠",
        images: ["images/2412-bro.jpg"],
        actorReply: {
          originalText: "这么凶干嘛",
          translatedText: "왜 소리질러"
        }
      },
      {
        id: 24124523,
        username: "Jia一口鲫鱼饼_",
        originalText: "会和我们一起倒数吗",
        translatedText: "우리랑 같이 카운트다운 할 거야?",
        images: [],
        actorReply: {
          originalText: "可能在片场 我可以在心里倒数",
          translatedText: "아마 촬영장에 있을 것 같은데 마음속으로 같이 할게"
        }
      },
      {
        id: 24124525,
        username: "总是难拥晨",
        originalText: "啥时候开赛车",
        translatedText: "언제 레이싱카 탈거야",
        images: [],
        actorReply: {
          originalText: "[doge]",
          translatedText: "[doge]"
        }
      }
    ]
  }
];
